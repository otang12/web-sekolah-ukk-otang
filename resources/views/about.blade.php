...
{{-- ============================================================
     HERO — foto full-width + overlay
     ============================================================ --}}
<div class="hero-full-wrap container">
    <section class="hero-full" id="about-hero" data-bg="{{ asset('img/hero/hero-1.jpeg') }}">
    ...
    {{-- Tab navigasi mengambang, overlap ke hero --}}
    <nav class="ap-tabs">
    ...
    <div class="ap-sambutan">
        <div class="ap-sambutan__foto-wrap">
            <div class="ap-sambutan__ring">
                <img src="{{ asset('img/' . $sambutan['foto']) }}" alt="{{ $sambutan['nama'] }}" class="ap-sambutan__foto">
            </div>
            <h3 class="ap-sambutan__nama">{{ $sambutan['nama'] }}</h3>
            <span class="ap-sambutan__jabatan">{{ $sambutan['jabatan'] }}</span>
        </div>
...
<section class="ap-section ap-section--tint" id="profil-sekolah">
    <div class="container">
        ...
        <div class="ap-keunggulan-grid">
            @php $paletteIcon = ['coral','teal','navy']; @endphp
            @foreach($keunggulan as $i => $k)
                @php $warna = $paletteIcon[$i % count($paletteIcon)]; @endphp
                <div class="ap-keunggulan-card">
                    <span class="ap-keunggulan-card__num">0{{ $i + 1 }}</span>
                    <span class="ap-keunggulan-card__icon ap-keunggulan-card__icon--{{ $warna }}">{!! $k['icon'] !!}</span>
                    <h3>{{ $k['judul'] }}</h3>
                    <p>{{ $k['teks'] }}</p>
                </div>
            @endforeach
...
<section class="container ap-section" id="sejarah">
    <div class="ap-sejarah">
        ...
        <p class="ap-sejarah__text">{{ $sejarah }}</p>
    </div>
</section>
    ...
    <div class="container">
        ...
        <div class="ap-vismis">
            <div class="ap-vismis__col ap-vismis__col--light">
                ...
                <h3>Moto</h3>
                <p class="ap-vismis__moto">{{ $moto }}</p>
            </div>
    ...
    <div class="ap-struktur-grid">
        @foreach($strukturOrganisasi as $s)
            <div class="ap-struktur-item">
                @if(!empty($s['foto']))
                    <img src="{{ asset('storage/' . $s['foto']) }}" alt="{{ $s['nama'] }}" class="ap-struktur-item__foto">
                @else
                    <span class="ap-struktur-item__avatar">{{ mb_substr($s['nama'], 0, 1) }}</span>
                @endif
                <span class="ap-struktur-item__jabatan">{{ $s['jabatan'] }}</span>
                <span class="ap-struktur-item__nama">{{ $s['nama'] }}</span>
            </div>
        @endforeach
    </div>
</section>
