...
<head>
    ...
    <link rel="icon" type="image/jpeg" href="{{ asset('img/logo-sekolah.jpeg') }}">
...
<div class="dash2">
    ...
    <header class="dash2-nav">
        <div class="dash2-nav__brand">
            <img src="{{ asset('img/logo-sekolah.png') }}" alt="Logo SMK Negeri 1 Cijati">
            <span>SMK Negeri 1 Cijati</span>
        </div>

        <nav class="dash2-nav__menu">
            <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.dashboard') ? 'is-active' : '' }}">Ringkasan</a>
            <a href="{{ route('admin.jurusan.index') }}" class="{{ request()->routeIs('admin.jurusan.*') ? 'is-active' : '' }}">Jurusan</a>
            <a href="{{ route('admin.news.index') }}" class="{{ request()->routeIs('admin.news.*') ? 'is-active' : '' }}">Berita</a>
            <a href="{{ route('admin.guru.index') }}" class="{{ request()->routeIs('admin.guru.*') ? 'is-active' : '' }}">Guru &amp; Staf</a>
            <a href="{{ route('admin.galeri.index') }}" class="{{ request()->routeIs('admin.galeri.*') ? 'is-active' : '' }}">Galeri</a>
            <a href="{{ route('admin.ekstrakurikuler.index') }}" class="{{ request()->routeIs('admin.ekstrakurikuler.*') ? 'is-active' : '' }}">Ekstrakurikuler</a>
        </nav>
        ...
        <div class="dash2-nav__account">
            ...
            <img src="{{ asset('img/logo-sekolah.png') }}" alt="" class="dash2-nav__avatar">
    ...
    <main class="dash2-body">
        ...
        {{--
            CONTOH STATIS. Ganti dengan data asli dari controller
            (mis. $stats['siswa'], $stats['guru'], dst) begitu tabelnya siap.
        --}}
        @php
            $statCards = [
        ...
        <section class="dash2-hero">
            <div>
                <span class="dash2-hero__date">{{ now()->translatedFormat('l, j F Y') }}</span>
                <h1>Selamat datang, {{ auth()->user()->name }} 👋</h1>
                <p>Berikut ringkasan aktivitas sekolah hari ini.</p>
            </div>
            <div class="dash2-hero__photo">
                <img src="{{ asset('img/hero/hero-1.jpeg') }}" alt="Gerbang SMK Negeri 1 Cijati">
            </div>
        </section>
        ...
        <div class="dash2-columns">
            ...
            <div class="dash2-stats-col">
                <div class="dash2-stats">
                    @foreach($statCards as $i => $card)
                        <div class="dash2-stat">
                            <span class="dash2-stat__value">{{ $card['value'] }}</span>
                            <span class="dash2-stat__label">{{ $card['label'] }}</span>
                            <span class="dash2-stat__note">{{ $card['note'] }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="dash2-quicklinks">
                    @foreach($quickLinks as $link)
                        <a href="{{ route($link['route']) }}" class="dash2-quicklink">
                            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">{!! $link['icon'] !!}</svg>
            ...
            <aside class="dash2-timeline-col">
                ...
                <ol class="dash2-timeline">
                    @foreach($aktivitasContoh as $item)
                        <li>
