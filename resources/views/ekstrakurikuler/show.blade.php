...
<div class="ekm3">
    ...
    <div class="container ekm3-topbar">
        <a href="{{ route('ekstrakurikuler') }}" class="ekm3-back">&larr; Semua Ekstrakurikuler</a>
    </div>

    <header class="container ekm3-banner">
        <div class="ekm3-banner__media" id="ekm3BannerMedia" @if($mainPhoto) data-bg="{{ asset('storage/' . $mainPhoto->file) }}" @endif>
    ...
    <div class="container ekm3-panels">
        ...
        <section class="ekm3-panel is-active" data-panel="tentang">
            <p class="ekm3-lead">{{ $ekskul->deskripsi }}</p>
            <div class="ekm3-cta">
                <div>
                    <h3>Tertarik ikut {{ $ekskul->nama }}?</h3>
                    <p>Kembangkan minat dan bakatmu bersama teman-teman lainnya.</p>
                </div>
                <a href="{{ route('ekstrakurikuler') }}" class="ekm3-cta__btn">Lihat Lainnya &rarr;</a>
            </div>
        </section>
        ...
        <section class="ekm3-panel" data-panel="jadwal">
            <div class="ekm3-facts">
                <div class="ekm3-fact">
                    ...
                    <strong>{{ $ekskul->jadwal_hari ?? 'Setiap Sabtu' }}</strong>
                </div>
                <div class="ekm3-fact">
                    <span class="ekm3-fact__label">Waktu</span>
                    <strong>{{ $ekskul->jadwal_waktu ?? 'Pukul 14.00 - 16.30 WIB' }}</strong>
                </div>
                <div class="ekm3-fact">
                    <span class="ekm3-fact__label">Lokasi</span>
                    <strong>{{ $ekskul->jadwal_lokasi ?? 'Lapangan SMK N 1 Cijati' }}</strong>
                </div>
            </div>
            <div class="ekm3-tags">
                ...
                <div class="ekm3-gallery">
                    @foreach($ekskul->fotos as $foto)
                        <div class="ekm3-gallery__item">
                            <img src="{{ asset('storage/' . $foto->file) }}" alt="Kegiatan {{ $ekskul->nama }}" loading="lazy">
                        </div>
                    @endforeach
                </div>
            </section>
        ...
        <section class="ekm3-panel" data-panel="pembina">
            <div class="ekm3-pembina">
                <div class="ekm3-pembina__photo">
                    @if(!empty($ekskul->pembina_foto))
                        <img src="{{ asset('storage/' . $ekskul->pembina_foto) }}" alt="Foto Pembina {{ $ekskul->nama }}">
                    @else
                        <span>&#128100;</span>
                    @endif
                </div>
                <strong>{{ $ekskul->pembina_nama ?? 'Nama pembina belum diisi' }}</strong>
                <span class="ekm3-pembina__role">{{ $ekskul->pembina_jabatan ?? 'Guru Pembina Ekstrakurikuler' }}</span>
                <blockquote class="ekm3-quote">
                    &ldquo;{{ $ekskul->pembina_sambutan ?? 'Selamat bergabung di ' . $ekskul->nama . '. Mari kita kembangkan semangat, kekompakan, dan prestasi bersama-sama.' }}&rdquo;
                </blockquote>
            </div>
        </section>
