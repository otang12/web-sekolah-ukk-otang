...
@section('body-class', 'page-home page-home--playful')

@section('content')

{{-- ============================================================
     HERO — foto full-width dengan overlay (gaya referensi)
     ============================================================ --}}
<section class="rhero" id="hero-slider"
    data-bg="{{ asset('img/hero/' . $heroImages[0]) }}"
    data-images='{{ json_encode(array_map(function ($img) { return asset('img/hero/' . $img); }, $heroImages)) }}'>
    ...
    <div class="rhero__overlay">
        <div class="container">
            ...
            <div class="rhero__actions">
                <a href="{{ route('news.index') }}" class="pbtn pbtn--sunny">Lihat Berita &amp; Kegiatan &rarr;</a>
                <a href="{{ route('about') }}" class="pbtn pbtn--ghost">Kenali Sekolah Kami</a>
            </div>
        </div>
    </div>
</section>
...
{{-- ============================================================
     QUICKLINKS — 3 kartu akses cepat, warna beda-beda
     ============================================================ --}}
<section class="container pquick">
    <div class="pquick__grid">
        <a href="{{ route('about') }}" class="pquick-card pquick-card--purple">
        ...
        <a href="{{ route('guru') }}" class="pquick-card pquick-card--coral">
            <span class="pquick-card__icon">🧑‍🎓</span>
            <h3>Guru &amp; Staf</h3>
            <p>Tenaga pengajar kompeten di bidangnya masing-masing.</p>
            <span class="pquick-card__arrow">&rarr;</span>
        </a>
        <a href="{{ route('ekstrakurikuler') }}" class="pquick-card pquick-card--teal">
            <span class="pquick-card__icon">🎉</span>
...
{{-- ============================================================
     TOP TAGS — pill warna-warni
     ============================================================ --}}
<section class="container ptags">
    <span class="ptags__label"># Top Tags</span>
    <div class="ptags__list">
        @foreach($tags as $i => $tag)
            @php $colors = ['coral','sky','sunny','mint','purple','teal']; $c = $colors[$i % count($colors)]; @endphp
            <span class="ptag-pill ptag-pill--{{ $c }}">{{ $tag }}</span>
        @endforeach
    </div>
</section>

{{-- ============================================================
     PROGRAM KEAHLIAN — kartu playful, data dari admin
     ============================================================ --}}
<section class="container psection">
    ...
    <div class="pjurusan-grid">
        ...
        @foreach($kartuHome as $k)
            ...
            <a href="{{ $tujuan }}" class="pjurusan-card pjurusan-card--{{ $warna }}">
                <span class="pjurusan-card__badge">
                    @if($j && $j->logo)
                        <img src="{{ asset('storage/' . $j->logo) }}" alt="{{ $k['judul'] }}">
                    @else
                        <span>{{ $k['singkatan'] }}</span>
                    @endif
                </span>
                <h3>{{ $j->nama ?? $k['judul'] }}</h3>
                <p>{{ $j->deskripsi ?? $k['teks'] }}</p>
    ...
    <div class="psection__more">
        <a href="{{ route('jurusan.index') }}" class="pbtn pbtn--outline-purple">Lihat Semua Program Keahlian</a>
    </div>
</section>

{{-- ============================================================
     EKSTRAKURIKULER — carousel bulat warna-warni
     ============================================================ --}}
<section class="container psection psection--tint">
    ...
    <div class="pekskul-scroll" id="ekskulScroll">
        @php $colors = ['coral','sky','sunny','mint','purple','teal']; @endphp
        @forelse($ekstrakurikulerList as $i => $e)
            @php $c = $colors[$i % count($colors)]; @endphp
            <a href="{{ route('ekstrakurikuler.show', $e->slug) }}" class="pekskul-card pekskul-card--{{ $c }}">
                <span class="pekskul-card__icon">
                    @if($e->logo)
                        <img src="{{ asset('storage/' . $e->logo) }}" alt="{{ $e->nama }}">
                    @else
                        <span>{{ substr($e->nama, 0, 1) }}</span>
                    @endif
                </span>
                <h4>{{ $e->nama }}</h4>
            </a>
    ...
    <div class="psection__more">
        <a href="{{ route('ekstrakurikuler') }}" class="pbtn pbtn--outline-teal">Lihat Semua Ekstrakurikuler</a>
    </div>
</section>

{{-- ============================================================
     TENTANG KAMI — ringkas, playful
     ============================================================ --}}
<section class="container pabout">
    <div class="pabout__grid">
        <div class="pabout__photo">
            <img src="{{ asset('img/hero/' . $heroImages[0]) }}" alt="Gerbang SMK Negeri 1 Cijati">
        ...
        <div class="pabout__text">
            ...
            <a href="{{ route('about') }}" class="pbtn pbtn--sunny">Selengkapnya &rarr;</a>
        </div>
    </div>
</section>
...
<section class="container psection">
    <div class="psection__head psection__head--split">
        ...
        <a href="{{ route('news.index') }}" class="pbtn pbtn--outline-coral">Lihat Semua &rarr;</a>
    </div>
    ...
    <div class="pnews-grid">
        ...
        @forelse($latestNews as $i => $item)
            @php
                $label = $categoryLabels[$item->category] ?? 'Info Pendidikan';
                $c = $colors[$i % count($colors)];
            @endphp

            <article class="pnews-card pnews-card--{{ $c }}">
                <a href="{{ route('news.show', $item->slug) }}" class="pnews-card__thumb">
                    @if($item->image)
                    ...
                    @endif
                    <span class="pnews-card__badge">{{ $label }}</span>
                </a>
                ...
                <div class="pnews-card__body">
                    ...
                    <div class="pnews-card__meta">
                        <span>🗓️ {{ optional($item->published_at)->translatedFormat('d F Y') }}</span>
