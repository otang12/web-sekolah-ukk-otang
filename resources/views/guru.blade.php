...
<section class="hero-full-wrap container">
    <div class="hero-full" data-hero-bg="{{ asset('img/hero/guru-hero.jpeg') }}">
        <div class="hero-full__overlay">
            <div class="container">
                <div class="hero-full__inner">
                    ...
                    <p class="hero__desc">{{ $gurus->count() }} tenaga pendidik &amp; kependidikan</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($kepsek)
        <div class="gped-kepsek gped-animate" id="kepala-sekolah">
            <div class="gped-kepsek__photo">
                @if($kepsek->foto)
                    <img src="{{ asset('storage/' . $kepsek->foto) }}" alt="{{ $kepsek->nama }}">
                @endif
            </div>
            <span class="gped-kepsek__eyebrow">Pimpinan Sekolah</span>
            <h2>{{ $kepsek->nama }}</h2>
            <span class="gped-kepsek__rule"></span>
            <span class="gped-kepsek__jabatan">{{ $kepsek->jabatan }}</span>
        </div>
    @endif
    ...
    @if($filterLabels->count() > 1)
        <div class="gped-filter gped-animate" role="tablist">
            ...
            @foreach($filterLabels as $label)
                <button type="button" class="gped-filter__btn" data-filter="{{ $label }}">{{ $label }}</button>
            @endforeach
        </div>
    @endif

    <div class="gped-grid" id="gpedGrid">
        @foreach($guruLain as $g)
            @php $accent = $getAccent($g->jabatan); @endphp
            <div class="gped-card gped-animate" data-jurusan="{{ $accent['label'] }}" style="--i: {{ $loop->index }}; --accent: {{ $accent['color'] }};">
                <div class="gped-card__photo">
                    @if($g->foto)
                        <img src="{{ asset('storage/' . $g->foto) }}" alt="{{ $g->nama }}" loading="lazy">
                    @endif
                </div>
                <div class="gped-card__body">
                    <h3>{{ $g->nama }}</h3>
                    <span class="gped-card__jabatan">{{ $g->jabatan }}</span>
                    <span class="gped-card__tag">{{ $accent['label'] }}</span>
                </div>
            </div>
        @endforeach
    </div>
